...
const NegotiationDetails = () => {
  ...
  const fetchNegotiation = useCallback(async (showLoader = true) => {
    try {
  ...
  const handleAccept = async () => {
    try {
      ...
      setError(
        err.response?.data?.message || "Failed to accept offer. Please retry."
      );
    } finally {
  ...
  const handleReject = async () => {
    try {
      ...
      setError(
        err.response?.data?.message || "Failed to reject offer. Please retry."
      );
    } finally {
      setActionLoading(false);
    }
    ...
    >
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div ref={gridRef} className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          {/* Right Column: Status & Direct Form Actions */}
                ...
                <div className="bg-white border border-[#E8DDCB] rounded-2xl p-6 shadow-xs space-y-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <button
                      ...
                      className="w-full py-3 px-4 bg-emerald-600 hover:bg-emerald-700 disabled:bg-emerald-200 text-white font-extrabold text-xs rounded-xl shadow-xs transition-all flex items-center justify-center gap-1.5 active:scale-95 cursor-pointer disabled:cursor-not-allowed"
                    >
                      <svg
